...
def validate_tsv_field_count(file_path: Path, expected_count: int) -> Tuple[bool, int]:
    """
    Validate that the TSV file has the expected number of fields.
    
    Checks the first non-empty line to determine field count.
    
    Args:
        file_path: Path to TSV file
        expected_count: Expected number of fields (columns)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if not line:  # Skip empty lines
                    continue
                # Count fields (tab-separated)
                actual_count = len(line.split('\t'))
                return actual_count == expected_count, actual_count
        # If file is empty, return False
        return False, 0
    except Exception as e:
        logger.error(f"Error validating field count in {file_path}: {e}")
        raise ValueError(f"Could not validate field count in {file_path}: {e}")
...
def get_table_column_list(table_type: str) -> List[str]:
    ...
    column_lists = {
        ...
        "summarypage": [
            ...
            "TABLE_VALUE_TOTAL", "ISCONFIDENTIALOMITTED"
        ],
        "othermanager": [
...
def load_data_file_fast(
    ...
    # Validate field count before loading
    expected_field_count = len(columns)
    is_valid, actual_count = validate_tsv_field_count(file_path, expected_field_count)
    if not is_valid:
        error_msg = (
            f"Field count mismatch in {file_path.name}: "
            f"expected {expected_field_count} fields (columns), "
            f"but found {actual_count} fields in TSV file. "
